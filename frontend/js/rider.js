...
function displaySearchResults(results) {
    ...
    container.innerHTML = `
        <div class="current-location-card" onclick="selectCurrentLocation()">
            ...
            <div class="current-location-arrow">
...
async function confirmBooking() {
    ...
    try {
        ...
        if (data.success) {
            ...
            localStorage.setItem('rapidRide_currentTrip', JSON.stringify(currentTrip));
            showSearchingSheet();
...
function handleDriverAssigned(data) {
    ...
    // Update vehicle icon
    const vehicleType = currentTrip?.vehicleType || selectedVehicleType || 'car';
    const vehicleIconFn = document.getElementById('vehicle-icon');
    if (vehicleIconFn) {
        vehicleIconFn.className = 'fas'; // Reset
        if (vehicleType === 'bike') {
            vehicleIconFn.classList.add('fa-motorcycle');
        } else if (vehicleType === 'auto') {
            vehicleIconFn.classList.add('fa-taxi');
        } else {
            vehicleIconFn.classList.add('fa-car');
        }
    }
    ...
    if (window.mapFunctions && data.driver?.location) {
        window.mapFunctions.addMarker(
            ...
            'driver',
            currentTrip?.vehicleType || selectedVehicleType || 'car'
        );
    }
}
...
function updateDriverLocation(data) {
    ...
    if (window.mapFunctions && data.lat && data.lng) {
        window.mapFunctions.updateDriverMarker(data.lat, data.lng, currentTrip?.vehicleType || selectedVehicleType || 'car');
    }
}
    ...
    if (statusTextEl) {
        switch (data.status) {
            ...
            case 'trip_completed':
                ...
                // Hide driver sheet and show payment modal
                document.getElementById('driver-sheet').style.display = 'none';
                document.getElementById('location-sheet').style.display = 'block';

                const completedTripId = data.tripId || (currentTrip ? currentTrip._id : null);
                const completedFare = (currentTrip && currentTrip.fare) ? (currentTrip.fare.amount || currentTrip.fare) : 0;

                showNotification('Trip completed! Please make payment.', 'success');
                setTimeout(() => {
                    if (completedTripId) {
                        showPaymentModal(completedTripId, completedFare);
                    }
                }, 1000);
...
// ==================== PAYMENT ====================
function showPaymentModal(tripId, fare) {
    currentPaymentTripId = tripId;
    document.getElementById('payment-modal').style.display = 'flex';
    document.getElementById('payment-amount-display').textContent = '₹' + Math.round(fare);

...
async function searchLocationIQ(query) {
    try {
        ...
        const exactMatches = formattedResults.filter(result =>
            result.name.toLowerCase().trim() === query.toLowerCase().trim()
        );
...
function getVehicleIcon(type) {
    ...
    return `<div class="map-marker driver-marker"><i class="fas ${iconClass}"></i></div>`;
}

function addMarker(lat, lng, type, vehicleType = 'car') {
    ...
    if (type === 'pickup') {
        ...
        iconHtml = getVehicleIcon(vehicleType);
    ...
    if (type === 'driver') {
        markers[type].vehicleType = vehicleType;
    }
}
...
function updateDriverMarker(lat, lng, vehicleType = 'car') {
    if (markers.driver) {
        // If vehicle type changed, recreate marker
        if (markers.driver.vehicleType && markers.driver.vehicleType !== vehicleType) {
            addMarker(lat, lng, 'driver', vehicleType);
            return;
        }

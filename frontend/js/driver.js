...
// Stats - will be initialized with driver-specific keys
let todayEarnings = 0;
let todayTrips = 0;
let onlineHours = 0;
let driverId = null; // Will be set when driver is fetched

// Get driver-specific localStorage key
function getStatKey(key) {
    if (!driverId) return null;
    return `driver_${driverId}_${key}`;
}

// Initialize stats with driver-specific keys
function initializeStats() {
    if (driverId) {
        todayEarnings = parseInt(localStorage.getItem(getStatKey('todayEarnings'))) || 0;
        todayTrips = parseInt(localStorage.getItem(getStatKey('todayTrips'))) || 0;
        onlineHours = parseFloat(localStorage.getItem(getStatKey('onlineHours'))) || 0;
    }
}

// Save stats to localStorage with driver-specific keys
function saveStats() {
    if (driverId) {
        localStorage.setItem(getStatKey('todayEarnings'), todayEarnings);
        localStorage.setItem(getStatKey('todayTrips'), todayTrips);
        localStorage.setItem(getStatKey('onlineHours'), onlineHours);
    }
}
...
async function fetchTodayStats() {
    try {
        const res = await fetch(`${API_URL}/analytics/today`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();

        if (data.success) {
            todayEarnings = data.data.earnings || 0;
            todayTrips = data.data.trips || 0;
            // onlineHours is tracked by frontend
            saveStats();
            updateStats();
        }
    } catch (err) {
...
function updateStats() {
    ...
    document.getElementById('online-hours').textContent = onlineHours.toFixed(1) + 'h';
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (driverData.success) {
                ...
                driverId = driver._id; // Set driver ID for localStorage keys
                initializeStats(); // Initialize stats with driver-specific keys
                fetchTodayStats(); // Fetch today's stats from backend
